...
const NetworkContainer = ({ algo, apiPort, apiToken, apiId, pools, update, add, edit, remove }) => (
  <Network type={KIND_NVIDIA_LEGACY}
           algo={algo}
           apiPort={apiPort}
           apiToken={apiToken}
           apiId={apiId}
...
const mapStateToProps = state => ({
  algo:     state.config[KIND_NVIDIA_LEGACY].algo,
  apiPort:  state.config[KIND_NVIDIA_LEGACY].apiPort,
  apiToken: state.config[KIND_NVIDIA_LEGACY].apiToken,
  apiId:    state.config[KIND_NVIDIA_LEGACY].apiId,
  pools:    state.config[KIND_NVIDIA_LEGACY].pools
});
...
const mapDispatchToProps = (dispatch, ownProps) => ({
  ...
  add(algo) {
    AddPoolModal.show(KIND_NVIDIA_LEGACY, algo, false, dispatch);
  },
  edit(pool) {
    EditPoolModal.show(KIND_NVIDIA_LEGACY, pool, dispatch);
  },
  remove(pool) {
    DeletePoolModal.show(KIND_NVIDIA_LEGACY, pool, dispatch);
  },
});
